...
# All Tasks including YouTube Subscribe and WhatsApp Channel
ALL_TASKS = [
    {"id": "yt_sub", "title": "🔔 Subscribe to YouTube", "url": "https://youtube.com/@beaconofislam", "reward": 0.2, "duration": 15},
    {"id": "wa_join", "title": "📱 Join WhatsApp", "url": "https://chat.whatsapp.com/EZgSS4NS4vKB6Uqtx1NiTH", "reward": 0.2, "duration": 15},
    {"id": "yt_1", "title": "▶️ Watch YT Video 1", "url": "https://youtube.com/shorts/M4PF2V7TOqE", "reward": 0.2, "duration": 60},
    {"id": "yt_2", "title": "▶️ Watch YT Video 2", "url": "https://youtube.com/shorts/JfinwV1CFuc", "reward": 0.2, "duration": 60},
    {"id": "yt_3", "title": "▶️ Watch YT Video 3", "url": "https://youtube.com/shorts/sT93F-rFmzY", "reward": 0.2, "duration": 60},
    {"id": "app_1", "title": "⬇️ Download App 1", "url": "https://playbb.fun/u/28229500", "reward": 1.0, "duration": 120},
    {"id": "app_2", "title": "⬇️ Download HiFami", "url": "https://s.hifamiapp.com/1/b6xaDAoXh", "reward": 0.9, "duration": 120}
]
...
def check_user_verification(user_id):
    """Checks the database to see if the user clicked the verification button."""
    res = db_fetch_one("SELECT verified FROM users WHERE user_id = ?", (user_id,))
    return res and res[0] == 1

def show_verification_gate(chat_id):
    """Shows the follow prompt for the Telegram Account"""
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("👤 Follow Account", url=f"https://t.me/{FOLLOW_ACCOUNT}"))
    markup.add(InlineKeyboardButton("✅ I Have Followed", callback_data="verify_membership"))
    
    bot.send_message(
        chat_id,
        "🔒 *Access Denied*\n\nYou must follow our official Telegram account to use this bot and start earning.",
        reply_markup=markup,
        parse_mode="Markdown"
    )
...
def handle_callbacks(call):
    ...
    if call.data == "verify_membership":
        # Database verification update to bypass API limit on user accounts
        db_execute("UPDATE users SET verified = 1 WHERE user_id = ?", (user_id,))
        bot.answer_callback_query(call.id, "✅ Account Verified!")
        bot.delete_message(chat_id, call.message.message_id)
        show_dashboard(chat_id, user_id)
        return

    if not check_user_verification(user_id):
        bot.answer_callback_query(call.id, "❌ You must follow the account first!", show_alert=True)
        return
    ...
    if call.data == "menu_main":
        ...
        # UPDATED: Trust Wallet Recommendation
        text = (
            "💼 *WALLET SETUP*\n\n"
            "Please reply to this message with your USDT wallet address.\n\n"
            "💡 *Recommendation:* We highly suggest using *Trust Wallet* to receive your funds safely and quickly."
        )
        ...
        # UPDATED: Extended User Information Logic
        ...
        for task in ALL_TASKS:
            ...
            if task_state and task_state[0] == 1:
                ...
                btn_text = f"{task['title']} (${task['reward']:.2f})"
